            ...
            <div id="form"
                ...
                <form action="" method="post" enctype="multipart/form-data"
                    ...
                    <label for="gameDescription"
                        ...
                        <textarea id="gameDescription" rows="4" cols="50" class="text-center outline-none text-white bg-lightpurple-0 h-20 p-2 w-full"></textarea>
                    </label>
...
const alterar = (req, res) => {
    upload.fields([
        ...
        if (err) {
            console.error(err);
            return res.status(500).send('Erro ao processar a solicitação.').end();
        }

        const userId = req.body.id;
        const name = req.body.name;

        const profileBanner = req.files['profileBanner'] ? req.files['profileBanner'][0] : null;
        ...
        // Verifica se há arquivos em ambos os inputs
        if (profileBanner) {
            const { buffer } = profileBanner;
            cloudinary.uploader.upload_stream((error, result) => {
                if (error) {
                    sendErrorResponse('Erro ao fazer o upload da imagem do banner.');
                } else {
                    const { url } = result;
                    const sql = 'UPDATE users SET name = ?, profileBanner = ? WHERE id = ?';
                    con.query(sql, [name, url, userId], (err, result) => {
                        if (err) {
                            sendErrorResponse('Erro ao atualizar o banner no banco de dados.');
                        } else {
        ...
        if (profilePicture) {
            ...
            cloudinary.uploader.upload_stream((error, result) => {
                if (error) {
                    sendErrorResponse('Erro ao fazer o upload da imagem do perfil.');
                } else {
                    const { url } = result;
                    const sql = 'UPDATE users SET name = ?, profilePicture = ? WHERE id = ?';
                    con.query(sql, [name, url, userId], (err, result) => {
                        if (err) {
                            sendErrorResponse('Erro ao atualizar a imagem do perfil no banco de dados.');
                        } else {
        ...
        // Se não houver arquivos, apenas atualize o nome
        if (!profileBanner && !profilePicture) {
            const sql = 'UPDATE users SET name = ? WHERE id = ?';
            con.query(sql, [name, userId], (err, result) => {
                if (err) {
                    sendErrorResponse('Erro ao atualizar o nome no banco de dados.');
                } else {
